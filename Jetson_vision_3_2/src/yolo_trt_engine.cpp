...
{
  if (engine_path.empty()) throw std::runtime_error("engine_path is empty");
  if (!LoadEngine(engine_path)) throw std::runtime_error("Failed to load TensorRT engine");
...
bool YoloTrtEngine::LoadEngine(const std::string& engine_path)
{
  ...
  // 기대: in=1x3xHxW, out=1x300x6 (YOLOv8+NMS)
  {
    const std::vector<int> expect_in  = {1, 3, input_h_, input_w_};
    const std::vector<int> expect_out = {1, 300, 6};
    ...
    {
      std::cerr << "[ERR] Output dims mismatch. expected [1x300x6]\n";
      return false;
    }
  }
  ...
  // 입력 버퍼(float32 고정)
  in_elems_ = 1;
  for (int d = 0; d < in_dims.nbDims; ++d) in_elems_ *= static_cast<size_t>(in_dims.d[d]);
  CHECK_CUDA(cudaMalloc(&d_input_, in_elems_ * sizeof(float)));
...
void YoloTrtEngine::Postprocess(std::vector<Detection>& detections, int img_w, int img_h)
{
  ...
  // out = (1,300,6) -> total elems = 1800
  if (out_elems_ % 6 != 0) return;
  const int N = static_cast<int>(out_elems_ / 6);
  ...
  for (int i = 0; i < N; ++i)
  {
    ...
    float y2 = p[i * 6 + 3];
    float conf = p[i * 6 + 4];
    int cls = static_cast<int>(std::round(p[i * 6 + 5]));
    ...
    int bw = std::max(0, ix2 - ix1);
    int bh = std::max(0, iy2 - iy1);
