// vision/src/feature_extractor.cpp
//
// Pipeline step 7.
// 보정된 line 중심점들을 rule/policy 호환 8D feature로 변환한다.
// ball/goal/backboard/hurdle 같은 단일 object target은 이 feature 단계를 거치지 않는다.
...
FeatureExtractor::Features FeatureExtractor::Compute(const std::vector<cv::Point2f> &pts_px, const cv::Size &image_size,
                                                     bool previous_in_recovery, double vx_prev, double wz_prev,
  ...
  for (const auto &p : pts_px) {
    if (!std::isfinite(p.x) || !std::isfinite(p.y))
      continue;
    const float u = static_cast<float>(Clamp(static_cast<double>(p.x), 0.0, static_cast<double>(W - 1)));
    const float v = static_cast<float>(Clamp(static_cast<double>(p.y), 0.0, static_cast<double>(H - 1)));
  ...
  const int keep_n = std::min(std::max(1, cfg.max_centers), static_cast<int>(pts.size()));
  ...
  const double cx = (cfg.image_center_u >= 0.0) ? cfg.image_center_u : 0.5 * static_cast<double>(W);
  ...
  const bool enter = (f.n_visible <= cfg.recover_enter_nvis) || (std::abs(f.u_err_near) > cfg.recover_enter_u);
  const bool exit = (f.n_visible >= cfg.recover_exit_nvis) && (std::abs(f.u_err_near) < cfg.recover_exit_u);
  if (enter)
    in_recovery = true;
  if (exit)
    in_recovery = false;
  ...
  if (enough_pts) {
    const double alpha = in_recovery ? cfg.lookahead_alpha_recovery : cfg.lookahead_alpha_normal;
