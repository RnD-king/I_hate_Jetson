    ...
    def update_perception(self, local_x, local_y, base_z, roll, pitch, yaw, env_origins):  # 가시 점/비전 특징 업데이트
        ...
        for i in range(self.num_envs):
            ...
            if keep_n >= 2:
                ...
                finite = np.isfinite(vv) & np.isfinite(uu)
                vv = vv[finite]
                uu = uu[finite]

                fit_ok = vv.shape[0] >= 2
                if fit_ok:
                    dv = float(np.max(vv) - np.min(vv))
                    fit_ok = dv > 1e-6
                ...
                if fit_ok:
                    try:
                        a_fit, b_fit = np.polyfit(vv, uu, 1)
                ...
                if fit_ok:
                    ...
                    # Degenerate linear fit case: keep robust fallback.
