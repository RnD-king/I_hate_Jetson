...
// CUDA buffer 할당, 전처리 호출, enqueueV3, 후처리, NMS까지 담당.
// YOLO 핵심 파일
...
static bool DimsEqual(const nvinfer1::Dims &a, const std::vector<int> &b) {
  if (a.nbDims != static_cast<int>(b.size()))
    return false;
...
static std::vector<vision::Detection>
ApplyNms(const std::vector<vision::Detection> &dets_sorted_desc,
         float iou_thres, int max_keep) {
  ...
  for (const auto &d : dets_sorted_desc) {
    ...
    if (!suppress) {
      kept.push_back(d);
      if (max_keep > 0 && static_cast<int>(kept.size()) >= max_keep)
        break;
    }
  }
  return kept;
...
void YoloTrtEngine::Postprocess(std::vector<Detection> &detections, int img_w,
  ...
  if (!printed) {
    ...
    std::cout << "[Postprocess] conf_thres=" << conf_thres_
              << " max_conf=" << max_conf << " cand=" << cand
              << " det(before_nms)=" << detections.size() << std::endl;
  }
  ...
  constexpr float kNmsIouThres = 0.80f;
  ...
  // apply NMS
  {
    ...
    if (!nms_printed) {
      ...
      std::cout << "[NMS] iou_thres=" << kNmsIouThres
                << " max_keep=" << kMaxDetections << " before=" << before
